...
class MMHeap // Root is on Max level
{
    /**
...
public:
    ...
    MMHeap(Container zcontainer, Compare zcompare = Compare())
     ...
     *        stream.
     **/
     void dump(std::ostream & zout = std::cout) const
     {
        // zout << "{";
        if (empty())
            zout << "Heap is Empty";
