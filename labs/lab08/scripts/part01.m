...
plot_fft(c(t).*p2(t), theory_x2(f), 'x_{2}', '$$e^{-j\pi f\tau} \tau sinc((f-f_{c})\tau)$$');
plot_fft(c(t).*p3(t), theory_x3(f), 'x_{3}', '$$e^{j\pi f\tau} \tau sinc((f-f_{c})\tau)$$');

% FFT with Cosine Modulation
plot_fft(m(t).*p1(t), theory_w1(f), 'w_{1}',...
    '$$\tau sinc((f-f_{c})\tau) + \tau sinc((f+f_{c})\tau)$$');

plot_fft(m(t).*p2(t), theory_w2(f), 'w_{1}',...
    '$$e^{-j\pi f\tau} (\tau sinc((f-f_{c})\tau) + \tau sinc((f+f_{c})\tau))$$');

plot_fft(m(t).*p3(t), theory_w3(f), 'w_{1}',...
    '$$e^{j\pi f\tau} (\tau sinc((f-f_{c})\tau) + \tau sinc((f+f_{c})\tau))$$');
